	...
	$.fn.editBox = function(B) {
		...
		var B = $.extend({
			...
			textoption:[
				...
				"收件人地址{address}",
				"目的地{province}",
				"寄件人{from_site_company}",
				"寄件人电话{from_tel}",
				"寄件人地址{from_address}",
				"日期{dateline}",
				"应付订单金额{payable_amount}",
				"实付金额{real_amount}",
				"备注{postscript}",
				"保价{insured}"
			],
		...
		var addCanvas = function(){
            ...
            $.each(B.deliveryOption, function(index, val){
                _bgtool.find("ol").append("<li id='"+ val.enname +"' data-id='"+ val.id +"'>"+ val.name +"</li>");
            })
		...
		var writeProperty = function(){
			if(B.isFocusbox.length == 1){
				...
				if(_texttool.find("span").text() !="自定义"){
					B.isFocusbox[0].find("ul").text(_texttool.find("span").text().replace(/[\u4e00-\u9fa5]/g,''));
					B.isFocusbox[0].find("textarea").val(_texttool.find("span").text());
				}
