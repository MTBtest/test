...
class Verify {
    ...
    function __construct($width = 130, $height = 50, $fontSize = 20) {
        ...
        $rand = rand(0, 1);
        if ($rand == 0) {
			$this->font = DOC_ROOT  . 'statics/Verify/fonts' . DIRECTORY_SEPARATOR . 'ggbi.ttf';
        } else {
            $this->font = DOC_ROOT  . 'statics/Verify/fonts' . DIRECTORY_SEPARATOR . 'verdana.ttf';
        }
    }
    ...
    protected function creat_code() {
        ...
        $this->code = $code;
        session('verify', strtolower($this->code));
    }
    ...
    private function creat_font() {
        ...
        for ($i = 0; $i < $this->code_len; $i++) {
            ...
            imagettftext($this->img, $this->font_size, rand(-00, 20), $x * $i + rand(4, 6), $this->height / 1.4 + rand(3, 5), $this->font_color, $this->font, $this->code[$i]);
            if ($i == 0)
                $this->x_start = $x * $i + 5;
        }
    }
    ...
    private function creat_line() {
        ...
        $width = $this->width / 2.66 + rand(3, 5);
